...
x0 = [0.2 1/10000000];        % Valores das condições iniciais
tmin = 0;               % Início do intervalo
tmax = 3.6;          % Final do intervalo
...
f = @(t,X) [X(2) -(X(2)^2*(I2(X(2))*(Pl+el) + 4*I1(X(2))*ev) + Pl + Pv*(4*I1(X(2))*X(2)^2-1))/(X(1)*(I4(X(2))*(Pl+el)*X(2)^2 + I2(X(2))*(Pl+el) + (Pv+ev)*(I1(X(2))+I3(X(2))*X(2)^2)))];
